#include "demo_device.h"

/*
** Generic read data from device
*/
int32_t DEMO_ReadData(uart_info_t *device, uint8_t *read_data, uint8_t data_length)
{
    int32_t status             = OS_SUCCESS;
    int32_t bytes              = 0;
    int32_t bytes_available    = 0;
    uint8_t ms_timeout_counter = 0;

    /* Wait until all data received or timeout occurs */
    bytes_available = uart_bytes_available(device);
    while ((bytes_available < data_length) && (ms_timeout_counter < DEMO_CFG_MS_TIMEOUT))
    {
        ms_timeout_counter++;
        OS_TaskDelay(1);
        bytes_available = uart_bytes_available(device);
    }

    if (ms_timeout_counter < DEMO_CFG_MS_TIMEOUT)
    {
        /* Limit bytes available */
        if (bytes_available > data_length)
        {
            bytes_available = data_length;
        }

        /* Read data */
        bytes = uart_read_port(device, read_data, (uint32_t)bytes_available);
        if (bytes != bytes_available)
        {
            OS_printf("  DEMO_ReadData: Bytes read != to requested! \n");
            status = OS_ERROR;
        } /* uart_read */
    }
    else
    {
        status = OS_ERROR;
    } /* ms_timeout_counter */

    return status;
}

/*
** Generic command to device
** Note that confirming the echoed response is specific to this implementation
*/
int32_t DEMO_CommandDevice(uart_info_t *device, uint16_t cmd_code, uint16_t payload)
{
    int32_t status = OS_SUCCESS;
    int32_t bytes  = 0;
    uint8_t write_data[DEMO_DEVICE_CMD_SIZE];
    uint8_t read_data[DEMO_DEVICE_DATA_SIZE];

    /* Prepare command */
    write_data[0] = DEMO_DEVICE_HDR_0;
    write_data[1] = DEMO_DEVICE_HDR_1;
    write_data[2] = (uint8_t)(cmd_code >> 8);
    write_data[3] = (uint8_t)(cmd_code & 0xFF);
    write_data[4] = (uint8_t)(payload >> 8);
    write_data[5] = (uint8_t)(payload & 0xFF);
    write_data[6] = DEMO_DEVICE_TRAILER_0;
    write_data[7] = DEMO_DEVICE_TRAILER_1;

    /* Flush any prior data */
    status = uart_flush(device);
    if (status == UART_SUCCESS)
    {
        /* Write data */
        bytes = uart_write_port(device, write_data, DEMO_DEVICE_CMD_SIZE);
        #ifdef DEMO_CFG_DEBUG
            OS_printf("  DEMO_CommandDevice[%d] = ", bytes);
            for (uint32_t i = 0; i < DEMO_DEVICE_CMD_SIZE; i++)
            {
                OS_printf("%02x", write_data[i]);
            }
            OS_printf("\n");
        #endif
        if (bytes == DEMO_DEVICE_CMD_SIZE)
        {
            status = DEMO_ReadData(device, read_data, DEMO_DEVICE_CMD_SIZE);
            if (status == OS_SUCCESS)
            {
                /* Confirm echoed response */
                bytes = 0;
                while ((bytes < (int32_t)DEMO_DEVICE_CMD_SIZE) && (status == OS_SUCCESS))
                {
                    if (read_data[bytes] != write_data[bytes])
                    {
                        status = OS_ERROR;
                    }
                    bytes++;
                }
            } /* DEMO_ReadData */
            else
            {
                #ifdef DEMO_CFG_DEBUG
                    OS_printf("DEMO_CommandDevice - DEMO_ReadData returned %d \n", status);
                #endif
            }
        }
        else
        {
            #ifdef DEMO_CFG_DEBUG
                OS_printf("DEMO_CommandDevice - uart_write_port returned %d, expected %d \n", bytes, DEMO_DEVICE_CMD_SIZE);
            #endif
        } /* uart_write */
    } /* uart_flush*/
    else
    {
        OS_printf("DEMO_CommandDevice - uart_flush returned error %d \n", status);
    }
    return status;
}

/*
** Request housekeeping command
*/
int32_t DEMO_RequestHK(uart_info_t *device, DEMO_Device_HK_tlm_t *data)
{
    int32_t status = OS_SUCCESS;
    uint8_t read_data[DEMO_DEVICE_HK_SIZE];

    /* Command device to send HK */
    status = DEMO_CommandDevice(device, DEMO_DEVICE_REQ_HK_CMD, 0);
    if (status == OS_SUCCESS)
    {
        /* Read HK data */
        status = DEMO_ReadData(device, read_data, sizeof(read_data));
        if (status == OS_SUCCESS)
        {
            #ifdef DEMO_CFG_DEBUG
                OS_printf("  DEMO_RequestHK = ");
                for (uint32_t i = 0; i < sizeof(read_data); i++)
                {
                    OS_printf("%02x", read_data[i]);
                }
                OS_printf("\n");
            #endif

            /* Verify data header and trailer */
            if ((read_data[0] == DEMO_DEVICE_HDR_0) && (read_data[1] == DEMO_DEVICE_HDR_1) &&
                (read_data[6] == DEMO_DEVICE_TRAILER_0) && (read_data[7] == DEMO_DEVICE_TRAILER_1))
            {
                data->DeviceCounter |= read_data[2] << 8;
                data->DeviceCounter |= read_data[3];
                data->DeviceConfig  |= read_data[4] << 8;
                data->DeviceConfig  |= read_data[5];
                #ifdef DEMO_CFG_DEBUG
                    OS_printf("  Header  = 0x%02x%02x  \n", read_data[0], read_data[1]);
                    OS_printf("  Counter = 0x%04x      \n", data->DeviceCounter);
                    OS_printf("  Config  = 0x%04x      \n", data->DeviceConfig);
                    OS_printf("  Trailer = 0x%02x%02x  \n", read_data[6], read_data[7]);
                #endif
            }
            else
            {
                OS_printf("  DEMO_RequestHK: DEMO_ReadData reported error %d \n", status);
                status = OS_ERROR;
            }
        } /* DEMO_ReadData */
    }
    else
    {
        OS_printf("  DEMO_RequestHK: DEMO_CommandDevice reported error %d \n", status);
    }
    return status;
}

/*
** Request data command
*/
int32_t DEMO_RequestData(uart_info_t *device, DEMO_Device_Data_tlm_t *data)
{
    int32_t status = OS_SUCCESS;
    uint8_t read_data[DEMO_DEVICE_DATA_SIZE];

    /* Command device to send HK */
    status = DEMO_CommandDevice(device, DEMO_DEVICE_REQ_DATA_CMD, 0);
    if (status == OS_SUCCESS)
    {
        /* Read HK data */
        status = DEMO_ReadData(device, read_data, sizeof(read_data));
        if (status == OS_SUCCESS)
        {
            #ifdef DEMO_CFG_DEBUG
                OS_printf("  DEMO_RequestData = ");
                for (uint32_t i = 0; i < sizeof(read_data); i++)
                {
                    OS_printf("%02x", read_data[i]);
                }
                OS_printf("\n");
            #endif

            /* Verify data header and trailer */
            if ((read_data[0] == DEMO_DEVICE_HDR_0) && (read_data[1] == DEMO_DEVICE_HDR_1) &&
                (read_data[8] == DEMO_DEVICE_TRAILER_0) && (read_data[9] == DEMO_DEVICE_TRAILER_1))
            {
                data->Chan1 = read_data[2] << 8;
                data->Chan1 |= read_data[3];
                data->Chan2 = read_data[4] << 8;
                data->Chan2 |= read_data[5];
                data->Chan3 = read_data[6] << 8;
                data->Chan3 |= read_data[7];
                #ifdef DEMO_CFG_DEBUG
                    OS_printf("  Header  = 0x%02x%02x  \n", read_data[0], read_data[1]);
                    OS_printf("  Chan1   = 0x%04x, %d  \n", data->Chan1, data->Chan1);
                    OS_printf("  Chan2   = 0x%04x, %d  \n", data->Chan2, data->Chan2);
                    OS_printf("  Chan3   = 0x%04x, %d  \n", data->Chan3, data->Chan3);
                    OS_printf("  Trailer = 0x%02x%02x  \n", read_data[8], read_data[9]);
                #endif
            }
        }
        else
        {
            OS_printf("  DEMO_RequestData: Invalid data read! \n");
            status = OS_ERROR;
        } /* DEMO_ReadData */
    }
    else
    {
        OS_printf("  DEMO_RequestData: DEMO_CommandDevice reported error %d \n", status);
    }
    return status;
}
