...
*/
int32_t UT_CRYPTO_TC_ProcessSecurity_ReturnValue = CRYPTO_LIB_SUCCESS;
int32_t UT_CRYPTO_TM_ApplySecurity_ReturnValue = CRYPTO_LIB_SUCCESS;
int32_t UT_CRYPTO_SC_Init_ReturnValue = CRYPTO_LIB_SUCCESS;
...
*/
int32_t Crypto_TC_ProcessSecurity(uint8_t *tc_frame, int *frame_size, TC_t *tc_struct)
{
    ...
    if (tc_struct != NULL && tc_frame != NULL && frame_size != NULL)
    {
        ...
        tc_struct->tc_pdu_len = (uint16_t)*frame_size;
...
*/
int32_t Crypto_TM_ApplySecurity(uint8_t *tm_frame, uint16_t frame_len)
{
    if (!UT_CRYPTO_Enable_Stubs)
        return CRYPTO_LIB_ERROR;
...
SaInterface get_sa_interface_inmemory(void)
{
    ...
    static SaInterfaceStruct stub_interface = {0};
 ...
 */
void UT_CRYPTO_Set_SaHook(int32_t (*hook)(uint8_t, uint16_t, uint16_t, uint8_t, SecurityAssociation_t **))
{
...
int32_t UT_CRYPTO_Sa_Default(uint8_t gvcid, uint16_t scid, uint16_t vcid, uint8_t mapid,
