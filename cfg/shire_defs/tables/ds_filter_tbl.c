...
/*
** Reminder: 
**   Each filtering type relies on three filter factors, N, X, and O.
**   N of X messages will be stored starting at offset O.
**
**   Sequence based filtering - This type of filtering uses the packet sequence
**     number to determine if the message should be filtered or not. If X is 
**     greater than zero, the message shall be stored if and only if the 
**     sequence number modulo X is less than N. For example, N=4, X=6 and O=2 
**     will store 4 of 6 messages starting at offset 2.
*/
