...
A space system usually has a space segment and a ground segment.
The space segment can contain spacecraft and relays.
The ground segment can contain mission operations centers, science operations centers, and ground stations.
...
Telemetry and commands commonly use packet and frame formats such as those defined by CCSDS.
The current SHIRE ground and flight configuration includes CCSDS packet handling and CFDP services.
CFDP Class 2 adds acknowledgements and retransmission to its file transfer flow.

The Radio model provides Sleep, Transmit, Receive, and Duplex modes.
The DRM uses RTS 6 to create a fixed pass window by selecting Duplex mode for 480 scheduler wakeups before returning to Receive mode.
...
SHIRE uses the Demo component as its reference payload.
The Demo application and simulator provide commands, housekeeping, and three representative data channels.
